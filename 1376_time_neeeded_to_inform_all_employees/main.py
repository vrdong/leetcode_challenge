...
class Solution:
    def numOfMinutes(self, n: int, headID: int, manager: List[int], informTime: List[int]) -> int:
        ...
        for i in range(n):
            if manager[i] == -1:
                continue
            inferior[manager[i]] += [i]

        def getInformTime(i):
            
            current_inform_time = informTime[i]
            inferior_inform_time = 0
            for j in inferior[i]:
                current_inferior_inform_time = getInformTime(j)
                if inferior_inform_time < current_inferior_inform_time:
                    inferior_inform_time = current_inferior_inform_time
                    
            print(i, current_inform_time + inferior_inform_time)
            return current_inform_time + inferior_inform_time

        return getInformTime(headID)
        # return None
